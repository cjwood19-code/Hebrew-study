const D=window.TRAINER_DATA,vocab=D.vocab,sentences=D.sentences,verbs=D.verbs||[],connectors=D.connectors||[],matches=D.matches||[],opposites=D.opposites||[],SESSION_LEN=12,PASS=85;
const levelNames=["","Hebrew → English","English → Hebrew","Mixed recognition","Typed English meaning","Typed Hebrew recall","Sentence reading & comprehension","Hebrew matching & opposites","Connectors in context","Verbs: infinitive + שם פעולה + past/present/future","Sentence building","Apple Pencil handwriting","Mixed cumulative challenge"];
let state=JSON.parse(localStorage.getItem("hebrewTrainerState")||"null")||{level:1,best:{},missed:{},history:[],session:null,focus:false};
let current=null,locked=false,tileAnswer=[];
...
function sentenceQuestion(){
  let s=rand(sentences),words=s.he.split(" "),candidates=words.map(function(w,i){return{w:w,i:i}}).filter(function(x){return x.w.length>1&&!["אני","אתה","את","הוא","היא","אנחנו","הם","הן","יש"].includes(x.w)});
  let chosen=rand(candidates.length?candidates:words.map(function(w,i){return{w:w,i:i}})),blanked=[...words];blanked[chosen.i]="___";
  let allWords=[];sentences.forEach(function(x){x.he.split(" ").forEach(function(w){if(w!==chosen.w&&!allWords.includes(w))allWords.push(w)})});
  ...
  let translationOptions=[s.en];shuffle(sentences.filter(function(x){return x.he!==s.he})).forEach(function(x){if(translationOptions.length<4&&!translationOptions.includes(x.en))translationOptions.push(x.en)});
...
function verbQuestion(typed){let v=rand(verbs),r=Math.random();
...
if(r<.40){let reverse=Math.random()<.35,correct=reverse?v.infinitive:v.actionNoun,prompt=reverse?"מה שם הפועל של שם הפעולה: "+v.actionNoun+" ?":"מה שם הפעולה של: "+v.infinitive+" ?",pool=verbs.filter(function(x){return x.infinitive!==v.infinitive}).map(function(x){return reverse?x.infinitive:x.actionNoun});return{type:typed?"verbTyped":"verbMC",cat:"פעלים • שם פעולה",prompt:prompt,correct:correct,options:typed?null:shuffle([correct,...shuffle(pool).slice(0,3)])}}
...
function makeQuestion(){let L=state.level,item=pickItem();if(L===1)return mc(item,false);if(L===2)return mc(item,true);if(L===3)return mc(item,Math.random()<.5);if(L===4)return{type:"typedEn",item:item,prompt:item.he,correct:item.en};if(L===5)return{type:"typedHe",item:item,prompt:item.en,correct:item.he};if(L===6)return sentenceQuestion();if(L===7)return matchQuestion();if(L===8)return connectorQuestion();if(L===9)return verbQuestion(Math.random()<.45);if(L===10){let s=rand(sentences);return{type:"tiles",cat:"בניית משפטים",prompt:s.en,correct:s.he,words:shuffle(s.he.split(" "))}}if(L===11)return{type:"hand",item:item,prompt:item.en,correct:item.he};let r=Math.random();if(r<.12)return mc(item,Math.random()<.5);if(r<.23)return{type:"typedHe",item:item,prompt:item.en,correct:item.he};if(r<.34)return{type:"typedEn",item:item,prompt:item.he,correct:item.en};if(r<.47)return sentenceQuestion();if(r<.59)return matchQuestion();if(r<.71)return connectorQuestion();if(r<.84)return verbQuestion(Math.random()<.5);if(r<.92)return{type:"hand",item:item,prompt:item.en,correct:item.he};let s=rand(sentences);return{type:"tiles",cat:"בניית משפטים",prompt:s.en,correct:s.he,words:shuffle(s.he.split(" "))}}
...
function makeUniqueQuestion(){
  ...
  let q=null,sig="";
  for(let i=0;i<60;i++){
    q=makeQuestion();sig=questionSignature(q);
...
function startSession(){state.session={answered:0,correct:0,level:state.level,seen:[]};save();renderHeader();nextQuestion()}
function renderHeader(){let s=state.session||{answered:0,correct:0};$("levelBadge").textContent="Level "+state.level;$("modeLabel").textContent=levelNames[state.level];$("sessionCount").textContent="Question "+Math.min(s.answered+1,SESSION_LEN)+" of "+SESSION_LEN;$("score").textContent=s.correct+" / "+s.answered;$("accuracy").textContent=(s.answered?Math.round(s.correct/s.answered*100):0)+"%";$("best").textContent=state.best[state.level]!==undefined?state.best[state.level]+"%":"—";$("reviewCount").textContent=reviewWords().length;$("progressBar").style.width=Math.min(100,s.answered/SESSION_LEN*100)+"%";renderReview()}
...
function record(ok){if(locked)return;locked=true;state.session.answered++;if(ok)state.session.correct++;if(current.item){let k=current.item.he;if(ok)state.missed[k]=Math.max(0,(state.missed[k]||0)-1);else state.missed[k]=(state.missed[k]||0)+1}save();renderHeader();$("nextBtn").textContent=state.session.answered>=SESSION_LEN?"Finish session":"Next";$("nextBtn").classList.remove("hidden")}
...
function englishMatch(a,c){let aa=norm(a),parts=c.split("/").map(norm);return parts.some(function(p){return aa===p||aa.includes(p)||p.includes(aa)})}
function checkTyped(){if(locked)return;let a=$("answerInput").value,ok=current.type==="typedEn"?englishMatch(a,current.correct):norm(a)===norm(current.correct);showFeedback(ok,ok?"Correct.":"Correct answer: "+current.correct);record(ok)}
...
function finishSession(){let s=state.session,p=Math.round(s.correct/s.answered*100),oldLevel=state.level;state.best[oldLevel]=Math.max(state.best[oldLevel]||0,p);state.history.push({date:new Date().toISOString(),level:oldLevel,score:p});let msg;if(p>PASS){state.focus=false;if(state.level<12){state.level++;msg=p+"% — advanced to Level "+state.level+": "+levelNames[state.level]+"."}else msg=p+"% — highest level cleared; cumulative practice continues."}else if(p>=70){state.focus=false;msg=p+"% — staying at Level "+state.level+". Missed words will appear more often."}else{state.focus=true;reviewWords().forEach(function(x){state.missed[x[0]]=x[1]+1});msg=p+"% — focused review at Level "+state.level+"."}state.session=null;save();$("message").textContent=msg;$("message").classList.remove("hidden");$("quizCard").innerHTML='<div style="text-align:center"><h2>Session complete</h2><p>'+msg+'</p><button class="primary" id="newSessionBtn">Start next session</button></div>';$("newSessionBtn").onclick=function(){location.reload()};renderHeader();window.scrollTo({top:0,behavior:"smooth"})}
