const D=window.TRAINER_DATA,vocab=D.vocab,sentences=D.sentences,verbs=D.verbs||[],connectors=D.connectors||[],matches=D.matches||[],opposites=D.opposites||[],grammarQuestions=D.grammarQuestions||[],classVerbQuestions=D.classVerbQuestions||[],SESSION_LEN=12,PASS=85;
...
function verbQuestion(typed){if(classVerbQuestions.length&&Math.random()<.45)return classVerbQuestion(typed);let v=rand(verbs),r=Math.random();
...
function makeQuestion(){let L=state.level,item=pickItem();if(L===1)return mc(item,false);if(L===2)return mc(item,true);if(L===3)return mc(item,Math.random()<.5);if(L===4)return{type:"typedEn",item:item,prompt:item.he,correct:item.en};if(L===5)return{type:"typedHe",item:item,prompt:item.en,correct:item.he};if(L===6)return sentenceQuestion();if(L===7)return matchQuestion();if(L===8)return Math.random()<.52?connectorQuestion():grammarQuestion();if(L===9)return verbQuestion(Math.random()<.45);if(L===10){let s=rand(sentences);return{type:"tiles",cat:"בניית משפטים",prompt:s.en,correct:s.he,words:shuffle(s.he.split(" "))}}if(L===11)return{type:"hand",item:item,prompt:item.en,correct:item.he};let r=Math.random();if(r<.12)return mc(item,Math.random()<.5);if(r<.23)return{type:"typedHe",item:item,prompt:item.en,correct:item.he};if(r<.34)return{type:"typedEn",item:item,prompt:item.he,correct:item.en};if(r<.47)return sentenceQuestion();if(r<.58)return matchQuestion();if(r<.67)return connectorQuestion();if(r<.76)return grammarQuestion();if(r<.86)return verbQuestion(Math.random()<.5);if(r<.94)return{type:"hand",item:item,prompt:item.en,correct:item.he};let s=rand(sentences);return{type:"tiles",cat:"בניית משפטים",prompt:s.en,correct:s.he,words:shuffle(s.he.split(" "))}}
